...
USBD_StatusTypeDef	USBD_LL_Init (USBD_HandleTypeDef *pdev)
{
	...
	hpcd_USB_OTG.Init.ep0_mps = DEP0CTL_MPS_64;
	...
	if (pdev->id == DEVICE_FS)
	{
	HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG, 64);
	HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG, 0, 64);
	HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG, 1, 192);
	}
	else if (pdev->id == DEVICE_HS)
	{
	HAL_PCD_SetRxFiFo(&hpcd_USB_OTG, 0x100);
	HAL_PCD_SetTxFiFo(&hpcd_USB_OTG, 0, 0x200);
	HAL_PCD_SetTxFiFo(&hpcd_USB_OTG, 1, 0x200);
	}
