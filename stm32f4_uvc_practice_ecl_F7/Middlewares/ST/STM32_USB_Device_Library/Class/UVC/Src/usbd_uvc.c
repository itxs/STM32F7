...
uint8_t gBuffer[UVC_IMG_WIDTH * UVC_IMG_HEIGHT * UVC_BPP / 8];
...
/* USB Video Class device Configuration Descriptor */
...
uint8_t USBD_UVC_CfgFSDesc[USB_UVC_CONFIG_DESC_TOTAL_SIZE]	__ALIGN_END =
{
	...
	WBVAL(0x0110), /* bcdVDC: UVC 1.1 */
	WBVAL(VC_DESC_TOTAL_SIZE), /* wTotalLength */
	DBVAL(0), /* dwClockFrequency: OBSOLETE in UVC 1.1*/
	1, /* bInCollection */
	1, /* baInterfaceNr(1) */
	...
	UVC_IMG_FORMAT,			/* GUID Format */
	UVC_BPP,								/* bBitsPerPixel: 16 for YUY2 */
	...
	WBVAL(UVC_IMG_WIDTH), /*	wWidth	*/
	WBVAL(UVC_IMG_HEIGHT), /*	wHeight	*/
	DBVAL(UVC_IMG_WIDTH * UVC_IMG_HEIGHT * UVC_BPP * UVC_FPS), /*	dwMinBitRate	*/
	DBVAL(UVC_IMG_WIDTH * UVC_IMG_HEIGHT * UVC_BPP * UVC_FPS), /*	dwMaxBitRate	*/
	DBVAL(UVC_IMG_WIDTH * UVC_IMG_HEIGHT * UVC_BPP / 8), /*	dwMaxVideoFrameBufSize	*/
	DBVAL(10000000 / UVC_FPS), /*	dwDefaultFrameInterval: for [UVC_FPS] FPS	*/
	1, /*	bFrameIntervalType	*/
	DBVAL(10000000 / UVC_FPS), /*	dwFrameInterval(1)	*/
...
VideoControl	videoCommitControl =
{
	...
	{0x01},							 // bFormatIndex
	{0x01},							 // bFrameIndex
	{DBVAL(10000000 / UVC_FPS),},	 // dwFrameInterval
	{0x00,0x00,},					 // wKeyFrameRate
	{0x00,0x00,},					 // wPFrameRate
	{0x00,0x00,},					 // wCompQuality
	{0x00,0x00,},					 // wCompWindowSize
	{0x00,0x00},						// wDelay
	{DBVAL(UVC_IMG_WIDTH * UVC_IMG_HEIGHT * UVC_BPP / 8)},				 // dwMaxVideoFrameSize
						...
						uint8_t cfgidx)
{
	...
	huvc->frame_count = 0;
	huvc->frame_buffer = (uint8_t*)gBuffer;
...
uint8_t	USBD_UVC_Setup (USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
	...
	case USB_REQ_TYPE_STANDARD:
		...
		case USB_REQ_GET_INTERFACE:
			USBD_CtlSendData (pdev, (uint8_t *)&huvc->interface, 1);
			break;
...
uint8_t	USBD_UVC_DataIn (USBD_HandleTypeDef *pdev, uint8_t epnum)
{
	...
	if (huvc->frame_bytes_count >= UVC_IMG_WIDTH * UVC_IMG_HEIGHT * UVC_BPP / 8)
	{
		huvc->frame_bytes_count = 0;
	...
	packet[0] = UVC_BPP / 8;
	packet[1] = huvc->frame_toggle_byte;
	uint32_t packet_size = UVC_BPP / 8;
	
	while (packet_size < UVC_IN_EP1_PACKET_SIZE && huvc->frame_bytes_count < UVC_IMG_WIDTH * UVC_IMG_HEIGHT * UVC_BPP / 8)
	{
		packet[packet_size++] = huvc->frame_buffer[huvc->frame_bytes_count++];
	}
